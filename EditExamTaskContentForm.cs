...
{
    public partial class EditExamTaskContentForm : Form
    {

        int firstHeadingNum = 1;//大题数计数器,默认有1题
        int questionStorageNum = 1;//当前大题下题库数计数器，默认当前条件下有1个题库
        private readonly List<int> questionStorageNumList = new List<int>();
        ...
        private string textColumns;//试卷分栏数
        ...
        //大题题目列表
        private List<string> questionName = null;

        //大题下面所有小题的每题分数
        private List<float> questionGrade = null;

        //大题下每小题的空行数
        private List<int> questionNextLineSpace = null;

        //大题完整列表，除了大题名称，包括了里面的所有东西
        private List<List<QuestionElement>> firstQuestionList;

        public int FirstHeadingNum { get => firstHeadingNum; set => firstHeadingNum = value; }
        public int QuestionStorageNum { get => questionStorageNum; set => questionStorageNum = value; }

        public List<int> QuestionStorageNumList => questionStorageNumList;
        ...
        /// 构造函数
        /// </summary>
        public EditExamTaskContentForm()
        {
        ...
        /// <summary>
        /// 重载构造函数，为了传递当前任务文件进来（不变）
        /// </summary>
        /// <param name="examPaperTask"></param>
        public EditExamTaskContentForm(ExamPaperTask examPaperTask) : this()
        {
            ExamPaperTask = examPaperTask;
            this.Initialization(ExamPaperTask);
        }

        /// <summary>
        /// 初始化方法，将参数里面的所有数据现实到界面上（已更新）
        /// </summary>
        /// <param name="examPaperTask"></param>
        /// <exception cref="NotImplementedException"></exception>
        private void Initialization(ExamPaperTask examPaperTask)
        {
            FirstHeadingNum = examPaperTask.QuestionName.Count;

            TitleName = examPaperTask.TitleName;

            MarginTop = examPaperTask.MarginTop;
            MarginBottom = examPaperTask.MarginBottom;
            MarginLeft = examPaperTask.MarginLeft;
            MarginRight = examPaperTask.MarginRight;

            Orientation = examPaperTask.Orientation;
            TextColumns = examPaperTask.TextColumns;
            PaperSize = examPaperTask.PaperSize;

            MainTitleFont = examPaperTask.MainTitleFont;
            MainTitleFontSize = examPaperTask.MainTitleFontSize;
            MainTitleFontStyle = examPaperTask.MainTitleFontStyle;
            MainTitleNextLineSpace = examPaperTask.MainTitleNextLineSpace;

            SubTitleFont = examPaperTask.SubTitleFont;
            SubTitleFontSize = examPaperTask.SubTitleFontSize;
            SubTitleFontStyle = examPaperTask.SubTitleFontStyle;
            SubTitleNextLineSpace = examPaperTask.SubTitleNextLineSpace;

            FirstHeadingTitleFont = examPaperTask.FirstHeadingTitleFont;
            FirstHeadingTitleFontSize = examPaperTask.FirstHeadingTitleFontSize;
            FirstHeadingTitleFontStyle = examPaperTask.FirstHeadingTitleFontStyle;
            FirstHeadingTitleNextLineSpace = examPaperTask.FirstHeadingTitleNextLineSpace;

            SecondHeadingTitleFont = examPaperTask.SecondHeadingTitleFont;
            SecondHeadingTitleFontSize = examPaperTask.SecondHeadingTitleFontSize;
            SecondHeadingTitleFontStyle = examPaperTask.SecondHeadingTitleFontStyle;
            SecondHeadingTitleNextLineSpace = examPaperTask.SecondHeadingTitleNextLineSpace;

            QuestionName = examPaperTask.QuestionName;
            QuestionGrade = examPaperTask.QuestionGrade;
            QuestionNextLineSpace = examPaperTask.QuestionNextLineSpace;

            FirstQuestionList = new List<List<QuestionElement>>();
            for (int i = 0; i < examPaperTask.FirstQuestionList.Count; i++)
            {
                QuestionStorageNumList.Add(examPaperTask.FirstQuestionList[i].Count);
                ...
                for (int j = 0; j < examPaperTask.FirstQuestionList[i].Count; j++)
                {
                    ...
                    qe.Path = examPaperTask.FirstQuestionList[i][j].path;
                    qe.Amount = examPaperTask.FirstQuestionList[i][j].amount;
                    qeList.Add(qe);
                }
                FirstQuestionList.Add(qeList);
            }
            QuestionStorageNum = QuestionStorageNumList[QuestionStorageNumList.Count - 1];
            DisplayInfoToEditForm();
        }

        /// <summary>
        /// 将数据显示到界面上（已更新）
        /// </summary>
        private void DisplayInfoToEditForm()
        {
            this.MainTitleTextBox.Text = TitleName;

            this.marginTopComboBox.Text = MarginTop.ToString();
            this.marginBottomComboBox.Text = MarginBottom.ToString();
            this.marginLeftComboBox.Text = MarginLeft.ToString();
            this.marginRightComboBox.Text = MarginRight.ToString();

            this.paperSizeComboBox.Text = PaperSize;
            this.textColumnsComboBox.Text = TextColumns;
            this.orientationComboBox.Text = Orientation;

            this.mainTitleFontLabel.Text = MainTitleFont;
            this.mainTitleFontSizeLabel.Text = MainTitleFontSize;
            this.mainTitleFontStyleLabel.Text = MainTitleFontStyle;
            this.mainTitleNextSpacingLineComboBox.Text = MainTitleNextLineSpace;

            this.subTitleFontLabel.Text = SubTitleFont;
            this.subTitleFontSizeLabel.Text = SubTitleFontSize;
            this.subTitleFontStyleLabel.Text = SubTitleFontStyle;
            this.subTitleNextSpacingLineComboBox.Text = SubTitleNextLineSpace;

            this.firstHeadingFontLabel.Text = FirstHeadingTitleFont;
            this.firstHeadingFontStyleLabel.Text = FirstHeadingTitleFontStyle;
            this.firstHeadingFontSizeLabel.Text = FirstHeadingTitleFontSize;
            this.firstHeadingNextSpacingLineComboBox.Text = FirstHeadingTitleNextLineSpace;

            this.secondHeadingFontLabel.Text = SecondHeadingTitleFont;
            this.secondHeadingFontStyleLabel.Text = SecondHeadingTitleFontStyle;
            this.secondHeadingFontSizeLabel.Text = SecondHeadingTitleFontSize;
            this.secondHeadingNextSpacingLineComboBox.Text = SecondHeadingTitleNextLineSpace;

            //Console.WriteLine("Init: LocationHeight = {0}", locationHeight);

            for (int i = 0; i < FirstHeadingNum; i++)
            {
                if (i > 0) { FirstHeadingPlus(i + 1, 1); }
                foreach (System.Windows.Forms.TextBox tb in contentPanel.Controls.OfType<System.Windows.Forms.TextBox>())
                { if (tb.Name == "firstHeadingTextBox" + (i + 1).ToString()) tb.Text = QuestionName[i]; }
                foreach (System.Windows.Forms.ComboBox cb in contentPanel.Controls.OfType<System.Windows.Forms.ComboBox>())
                { if (cb.Name == "gradeComboBox" + (i + 1).ToString()) cb.Text = QuestionGrade[i].ToString(); }
                foreach (System.Windows.Forms.ComboBox cb in contentPanel.Controls.OfType<System.Windows.Forms.ComboBox>())
                { if (cb.Name == "nextLineSpaceComboBox" + (i + 1).ToString()) cb.Text = QuestionNextLineSpace[i].ToString(); }

                for (int j = 0; j < FirstQuestionList[i].Count; j++)
                {
                    if (j > 0) { QuestionStoragePlus(i + 1, j + 1); }
                    foreach (System.Windows.Forms.ComboBox cbb in contentPanel.Controls.OfType<System.Windows.Forms.ComboBox>())
                    { if (cbb.Name == "amountComboBox" + (i + 1).ToString() + "_" + (j + 1).ToString()) cbb.Text = FirstQuestionList[i][j].Amount.ToString(); }

                    foreach (System.Windows.Forms.TextBox tb in contentPanel.Controls.OfType<System.Windows.Forms.TextBox>())
                    {
                        //Console.WriteLine("Before:tb.Name = {0}", tb.Name);
                        if (tb.Name == "pathTextBox" + (i + 1).ToString() + "_" + (j + 1).ToString())
                        {
                            //Console.WriteLine("After:tb.Name = {0}",tb.Name);
                            tb.Text = FirstQuestionList[i][j].Path;
                            ...
                            if (Utilities.CheckQuestionStorageFormat(tb.Text))
                            {
                                ...
                                foreach (System.Windows.Forms.ComboBox cbb in contentPanel.Controls.OfType<System.Windows.Forms.ComboBox>())
                                {
                                    if (cbb.Name == "amountComboBox" + (i + 1).ToString() + "_" + (j + 1).ToString())
                                    {
                                        int currentSelectedIndex = cbb.SelectedIndex;//旧值
                                        //Console.WriteLine("currentSelectedIndex:{0}", currentSelectedIndex);
                                        cbb.Items.Clear();
                                        cbb.Items.AddRange(Utilities.NumToStringList(num));
        ...
        /// 题型+的重载（已更新）
        /// </summary>
        /// <param name="i">大题的序号值</param>
        /// <param name="j">题库的序号值</param>
        private void FirstHeadingPlus(int i, int j)
        {
            //Console.WriteLine("Inner: i = {0} , j={1}", i, j);
            if (i == 19) return;//最多不超过19个大题,一般情况下用不上那么多，但还是有个限制
                                //Console.WriteLine("题库+ 开始时 firstHeadingNum:{0}", firstHeadingNum);

            Height += FIRST_HEADING_OFFSET;//程序外界面框也向下增加25的长度
            examPaperContentGroupBox.Height += FIRST_HEADING_OFFSET;//GroupBox框增加25的长度
            contentPanel.Height += FIRST_HEADING_OFFSET;

            locationHeight += FIRST_HEADING_OFFSET;//新的数据其实点位置向下移25
            ...
            firstHeadingName.Name = "firstHeadingTextBox" + i.ToString();
            ...
            path.Name = "pathTextBox" + i.ToString() + "_" + j.ToString();
            ...
            amountCombobox.Name = "amountComboBox" + i.ToString() + "_" + j.ToString();
            ...
            gradeCombobox.Name = "gradeComboBox" + i.ToString();
            ...
            nextLineSpaceCombobox.Name = "nextLineSpaceComboBox" + i.ToString();
        ...
        /// 题库+的重载（已更新）
        /// </summary>
        /// <param name="i">大题序号值</param>
        /// <param name="j">题库序号值</param>
        private void QuestionStoragePlus(int i, int j)
        {
            //if (questionStorageNum == 9) return;//单个题型最多不超过9个题库路径，已经很疯狂了,走程序就不限制了吧
            ...
            //改变窗体长度和Groupbox长度，数值经过计算
            Height += QUESTION_STORAGE_OFFSET;
            examPaperContentGroupBox.Height += QUESTION_STORAGE_OFFSET;
            ...
            IsDataValid();//完事后，进行合法性检查
            //Console.WriteLine("题库+ 结束之后 questionStorageNum:{0}", questionStorageNum);
            //for (int i = 0; i < questionStorageNumList.Count; i++)
            //{ Console.WriteLine("questionStorageList[{0}] = {1}", i, questionStorageNumList[i]); }
        }
        ...
        /// <summary>
        /// 题型+后，在当前界面上新增一条填写大题信息的内容栏，
        /// 并同时改变Groupbox和整个窗体的大小，并对相关参数进行复制操作（已更新）
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        ...
        private void YesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                //Console.WriteLine("QuestionName: 第{0}题:{1}", 1 + i, QuestionName[i]);
                //Console.WriteLine("QuestionGrade: 第{0}题:{1}", 1 + i, QuestionGrade[i]);
                //Console.WriteLine("QuestionNextLineSpace: 第{0}题:{1}", 1 + i, QuestionNextLineSpace[i]);

                for (int j = 0; j < FirstQuestionList[i].Count; j++)
                {
                    string pathname = FirstQuestionList[i][j].Path;
                    double amountname = FirstQuestionList[i][j].Amount;
                    //Console.WriteLine("question[{0}][{1}]: path:{2},  amount:{3}", i, j, pathname, amountname);
                }
            }
            ...
            ExamPaperTask.ReadEditExamTaskContentFormData(this);//调用ExamPaperTask方法将此信息框的所有信息对新变量赋值
            //使用委托变量将数据传回到主界面
        ...
        private void IsDataValid()
        {
            ...
            yesToolStripMenuItem.Enabled = true;//全部检查完成后没有问题，"确认"键可使用
            return;
        }
        ...
        private void ShouldAmountButtonEnabled(string tail)
        {
            ...
            foreach (System.Windows.Forms.TextBox tb in contentPanel.Controls.OfType<System.Windows.Forms.TextBox>())
            {
                ...
                {
                    //Console.WriteLine("tb.Name = {0}", "pathTextBox" + i.ToString() + "_" + j.ToString());//测试用
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void EditExamTaskContentForm_FormClosing(object sender, FormClosingEventArgs e) { MainForm.eetc = null; }
        ...
        private void MainTitleFontButtonLabel_Click(object sender, EventArgs e)
        {
            ...
            fontDialog.Font = new Font(mainTitleFontLabel.Text, float.Parse(mainTitleFontSizeLabel.Text));
        ...
        private void SubTitleFontButtonLabel_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                //Console.WriteLine(selectedFont.Name);
                //Console.WriteLine(selectedFont.Size);
                //Console.WriteLine(selectedFont.Style);
                //Console.WriteLine(selectedFont.Bold);
                //Console.WriteLine(selectedFont.Italic);
